...
const COLUMNS = [
  ...
  { key: "name_supplier", label: "Nama Supplier", width: "w-[180px]", align: "text-left" },
  { key: "perusahaan", label: "Perusahaan", width: "w-[160px]", align: "text-left" },
  { key: "alamat_perusahaan", label: "Alamat", width: null, align: "text-left" },
  { key: "no_telp", label: "No. Telepon", width: "w-[140px]", align: "text-left" },
...
function SkeletonRow({ index }) {
  return (
    <tr className={`animate-[tableFadeIn_0.3s_ease_${index * 50}ms_both]`}>
      ...
      <td className="px-4 py-3"><Skel width="w-48" height="h-3" /></td>
...
function DataRow({ item, rowNum, idx, onEdit, onDelete, isLoading }) {
  return (
    ...
    >
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-md bg-blue-50 border border-blue-200 flex items-center justify-center shrink-0">
            <MdLocalShipping size={12} className="text-blue-600" />
          </div>
          <span className="font-medium text-sm text-gray-900">{item.name_supplier}</span>
        </div>
      </td>
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          <MdBusiness size={14} className="text-gray-400 shrink-0" />
          <span className="text-sm text-gray-600">{item.perusahaan || "-"}</span>
        </div>
      </td>
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-600 font-mono">{item.no_telp || "-"}</span>
        </div>
      </td>
