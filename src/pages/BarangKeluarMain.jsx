/**
 * BarangKeluarMain.jsx
 *
 * Optimasi:
 * 1. Server-side search — debouncedSearch dikirim ke API; rawData.filter() dihapus
 * 2. prefetchBarangKeluarExports dihapus — diganti hover-intent di tombol ekspor
 * 3. triggerDownload diekstrak jadi helper
...
import { MdAdd, MdRefresh, MdFileDownload, MdSearch, MdOutbox } from "react-icons/md";
import {
  getBarangKeluarList,
  createBarangKeluar,
  updateBarangKeluar,
  deleteBarangKeluar,
  exportBarangKeluarPDF,
  exportBarangKeluarExcel,
  clearBarangKeluarExportCache,
} from "../services/barangKeluarService";
import { showToast } from "../utils/toast";
import BarangKeluarTable from "../components/feature/barangkeluar/BarangKeluarTable";
import BarangKeluarModal from "../components/feature/barangkeluar/BarangKeluarModal";
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function BarangKeluarMain() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getBarangKeluarList(page, limit, debouncedSearch);
  ...
  const handlePrefetchOnHover = (type) => {
    ...
    if (type === "pdf") exportBarangKeluarPDF().catch(() => {});
    else                exportBarangKeluarExcel().catch(() => {});
  };
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      if (isEdit && selectedData) {
        await updateBarangKeluar(selectedData.id, formData);
        showToast.success("Barang keluar berhasil diperbarui");
      } else {
        await createBarangKeluar(formData);
        showToast.success("Barang keluar berhasil ditambahkan");
      }
      clearBarangKeluarExportCache();
      resetPrefetchFlags();
      handleCloseModal();
  ...
  const handleConfirmDelete = async () => {
    try {
      setIsDeleting(true);
      await deleteBarangKeluar(deleteModal.id);
      showToast.success("Barang keluar berhasil dihapus");
      clearBarangKeluarExportCache();
      resetPrefetchFlags();
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const blob = await exportBarangKeluarPDF();
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const blob = await exportBarangKeluarExcel();
      ...
      <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between animate-[fadeDown_0.4s_cubic-bezier(0.16,1,0.3,1)_both]">
        <div className="flex items-start gap-3.5">
          <div className="w-11 h-11 rounded-xl bg-red-50 border border-red-200 flex items-center justify-center shrink-0">
            <MdOutbox className="w-5 h-5 text-red-600" />
          </div>
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Kelola data transaksi pengeluaran barang
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2.5 w-full sm:grid-cols-3 xl:flex xl:w-auto xl:flex-wrap xl:justify-end">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold text-white bg-red-600 hover:bg-red-700 shadow-md transition-all duration-150 hover:scale-105 active:scale-95 xl:w-auto"
          >
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden animate-[fadeUp_0.45s_cubic-bezier(0.16,1,0.3,1)_0.05s_both]">
        ...
        <div className="flex flex-col gap-3 p-3 sm:p-4 border-b border-gray-200 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative flex items-center w-full lg:max-w-90">
            <MdSearch
              className={`absolute left-3 w-4 h-4 transition-colors ${search ? "text-red-600" : "text-gray-400"}`}
            />
            <input
              type="text"
              placeholder="Cari kode barang, nama, cabang, ruangan…"
              value={search}
          ...
          <div className="flex flex-col gap-2 px-4 py-3 border-b border-gray-200 bg-gray-50/50 sm:flex-row sm:items-center sm:justify-between">
            <span className="text-[11px] text-gray-400 flex-1">
              {debouncedSearch
                ...
                : `Total ${meta.total} data barang keluar`}
            </span>
            {debouncedSearch && (
              <button
                onClick={() => setSearch("")}
                className="self-start text-[11px] font-semibold text-red-600 hover:text-red-700 sm:self-auto"
              >
                Hapus filter
              </button>
            )}
