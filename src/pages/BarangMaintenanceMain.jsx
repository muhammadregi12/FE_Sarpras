/**
 * BarangMaintenanceMain.jsx
 *
 * Optimasi:
 * 1. Server-side search — debouncedSearch dikirim ke API; rawData.filter() dihapus
 * 2. prefetchBarangMaintenanceExports dihapus — diganti hover-intent di tombol ekspor
 * 3. triggerDownload diekstrak jadi helper (tidak duplikasi)
 * 4. PageBtn diekstrak jadi sub-komponen
 * 5. resetPrefetchFlags dipanggil setelah mutasi data
 */

import { useState, useEffect, useRef, useCallback } from "react";
import { ToastContainer } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { MdAdd, MdRefresh, MdFileDownload, MdSearch, MdBuild } from "react-icons/md";
import {
  getBarangMaintenanceList,
  createBarangMaintenance,
  updateBarangMaintenance,
  updateStatusBarangMaintenance,
  deleteBarangMaintenance,
  exportBarangMaintenancePDF,
  exportBarangMaintenanceExcel,
  clearBarangMaintenanceExportCache,
} from "../services/barangmaintenanceService";
import { showToast } from "../utils/toast";
import BarangMaintenanceTable from "../components/feature/barangmaintenance/BarangMaintenanceTable";
import BarangMaintenanceModal, {
  BarangMaintenanceSelesaiModal,
} from "../components/feature/barangmaintenance/BarangMaintenanceModal";
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function BarangMaintenanceMain() {
  ...
  const [isSubmittingSelesai,setIsSubmittingSelesai]= useState(false);
  ...
  const [selesaiModal,       setSelesaiModal]       = useState({ open: false, data: null });
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getBarangMaintenanceList(page, limit, debouncedSearch);
      ...
      showToast.error(error?.response?.data?.message || "Gagal memuat data maintenance");
    } finally {
      setLoading(false);
    }
  ...
  const handlePrefetchOnHover = (type) => {
    ...
    if (type === "pdf") exportBarangMaintenancePDF().catch(() => {});
    else                exportBarangMaintenanceExcel().catch(() => {});
  };
  ...
  const handleOpenSelesai     = (item) => setSelesaiModal({ open: true, data: item });
  const handleCloseSelesai    = () => setSelesaiModal({ open: false, data: null });

  const handleSubmit = async (formData) => {
    try {
      setIsSubmitting(true);
      if (isEdit && selectedData) {
        await updateBarangMaintenance(selectedData.id, formData);
        showToast.success("Data maintenance berhasil diperbarui");
      } else {
        await createBarangMaintenance(formData);
        showToast.success("Data maintenance berhasil ditambahkan");
      }
      clearBarangMaintenanceExportCache();
      resetPrefetchFlags();
      handleCloseModal();
      ...
      showToast.error(error?.response?.data?.message || "Gagal menyimpan data maintenance");
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleSubmitSelesai = async (formData) => {
    if (!selesaiModal.data?.id) return;
    try {
      setIsSubmittingSelesai(true);
      await updateStatusBarangMaintenance(selesaiModal.data.id, formData);
      showToast.success("Status maintenance berhasil diperbarui");
      clearBarangMaintenanceExportCache();
      resetPrefetchFlags();
      handleCloseSelesai();
      fetchData();
    } catch (error) {
      showToast.error(error?.response?.data?.message || "Gagal memperbarui status maintenance");
    } finally {
      setIsSubmittingSelesai(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      setIsDeleting(true);
      await deleteBarangMaintenance(deleteModal.id);
      showToast.success("Data maintenance berhasil dihapus");
      clearBarangMaintenanceExportCache();
      resetPrefetchFlags();
      setDeleteModal({ open: false, id: null, name: null });
      fetchData();
    } catch (error) {
      showToast.error(error?.response?.data?.message || "Gagal menghapus data maintenance");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const blob = await exportBarangMaintenancePDF();
      triggerDownload(blob, `laporan-barang-maintenance-${Date.now()}.pdf`);
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const blob = await exportBarangMaintenanceExcel();
      triggerDownload(blob, `laporan-barang-maintenance-${Date.now()}.xlsx`);
  ...
  return (
    <div className="min-h-full bg-white p-3 sm:p-4 lg:p-6">
      ...
      <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between animate-[fadeDown_0.4s_cubic-bezier(0.16,1,0.3,1)_both]">
        <div className="flex items-start gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Kelola data maintenance dan penyelesaian perbaikan barang
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2.5 w-full sm:grid-cols-3 xl:flex xl:w-auto xl:flex-wrap xl:justify-end">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold text-white bg-orange-600 hover:bg-orange-700 shadow-md transition-all duration-150 hover:scale-105 active:scale-95 xl:w-auto"
          >
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden animate-[fadeUp_0.45s_cubic-bezier(0.16,1,0.3,1)_0.05s_both]">
        ...
        <div className="flex flex-col gap-3 p-3 sm:p-4 border-b border-gray-200 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative flex items-center w-full lg:max-w-90">
            <MdSearch
              className={`absolute left-3 w-4 h-4 transition-colors ${search ? "text-orange-600" : "text-gray-400"}`}
            />
            <input
              type="text"
              placeholder="Cari kode barang, nama barang, status, petugas..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full py-2 pl-9 pr-8 text-xs text-gray-900 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:border-orange-500 transition-colors"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
          ...
          <div className="flex flex-col gap-2 px-4 py-3 border-b border-gray-200 bg-gray-50/50 sm:flex-row sm:items-center sm:justify-between">
            <span className="text-[11px] text-gray-400 flex-1">
              {debouncedSearch
                ...
                : `Total ${meta.total} data maintenance`}
            </span>
            {debouncedSearch && (
              <button
                onClick={() => setSearch("")}
                className="self-start text-[11px] font-semibold text-orange-600 hover:text-orange-700 sm:self-auto"
              >
                Hapus filter
              </button>
            )}
        ...
        <BarangMaintenanceTable
          data={data}
          onEdit={handleOpenEdit}
          onDelete={handleDelete}
          onSelesai={handleOpenSelesai}
          isLoading={loading}
          page={page}
          limit={limit}
        />
      ...
      <BarangMaintenanceSelesaiModal
        isOpen={selesaiModal.open}
        data={selesaiModal.data}
        onClose={handleCloseSelesai}
        onSubmit={handleSubmitSelesai}
        isLoading={isSubmittingSelesai}
      />
